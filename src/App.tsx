    ...
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a friendly AI assistant. Keep responses brief and conversational, like a natural chat. Use simple language and show personality while staying helpful. Reply should be very short. Don't bore with long replies."
          },
          ...messages,
          newMessage
        ].map(m => ({
  ...
  const startListening = async () => {
    ...
    try {
          ...
          const wavBlob = await new Promise<Blob>((resolve) => {
            ...
            for (let i = 0; i < audioBuffer.length; i++) {
              for (let channel = 0; channel < numberOfChannels; channel++) {
                const sample = Math.max(-1, Math.min(1, channelData[channel][i] * 3.0));
      ...
      recordingTimeout.current = setTimeout(() => {
        if (mediaRecorder.current?.state === 'recording' && !isProcessingRef.current) {
          handleStopListening();
        }
      }, 30000);
        ...
        <div className="max-w-4xl mx-auto px-4 py-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className={`px-4 py-2 rounded-full text-sm font-medium ${
              ...
              status === 'listening' ? 'bg-blue-100 text-blue-700' :
